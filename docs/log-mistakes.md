...
- Next.js build failed due to `useSearchParams()` not being wrapped in a Suspense boundary on the OAuth callback page. Fixed by splitting into a server `page.tsx` wrapper + a client `OAuthCallbackClient.tsx`.
